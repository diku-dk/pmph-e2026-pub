...
"Programming Massively Parallel Hardware", 2026-2027.
...
Write a CUDA program with two functions that adds two large vectors, i.e., `C[i] = A[i] + B[i]`. Use single precision `float`. The first function should implement a serial map performed on the CPU; the second function should implement a parallel map in CUDA performed on the GPU.   Check that the element-wise result on CPU is equal to the one computed on the GPU (modulo an epsilon error, e.g., `fabs(cpu_res[i] - gpu-res[i]) < 0.000001` for all `i`), and print a VALID or INVALID message.  Check validation and measure performance for at least three datasets of small, medium and large vector length. E.g., small refers to length `20057`, medium to length `1000031` and large to vector length `100000009`.  *Make sure your program prints:*
...
*Hint:* when you measure the GPU time: 
...
** the memory throughput of your CUDA implementation (GB/sec) for the small, medium and large datasets. In case you are not running on the dedicated servers, please also report the peak memory bandwidth of your GPU hardware.
** in case you observe that the performance of the medium dataset is higher than the large dataset, please try to explain what can possibly be the reason for this?
